...
CREATE TABLE IF NOT EXISTS child_group_members (
 ...
 status VARCHAR(24) NOT NULL DEFAULT 'INVITED' CHECK(status IN ('INVITED','PARENT_PENDING','ACTIVE','DECLINED','LEFT','REMOVED')),
...
CREATE INDEX IF NOT EXISTS idx_child_group_members_child_status ON child_group_members(child_id,status,updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_child_group_members_group_status ON child_group_members(group_id,status,updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_child_group_messages_group_recent ON child_group_messages(group_id,group_message_id DESC) WHERE is_deleted=FALSE;
CREATE INDEX IF NOT EXISTS idx_group_message_reactions_message ON group_message_reactions(group_message_id,updated_at DESC);
