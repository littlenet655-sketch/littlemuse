...
export function QuizBreakCard({
  ...
  const [loading, setLoading] = useState(Boolean(token) && !completed);
  ...
  const loadRequiredQuiz = useCallback(async () => {
    if (!token || completed) {
      setLoading(false);
      return;
    }
    ...
    setLoadError(false);
    setQuiz(null);
    for (let attempt = 0; attempt < SERVER_LATCH_RETRIES; attempt += 1) {
      try {
        const response = await fetchQuiz(token, 1);
        if (response.required && response.reason === 'feed_break' && response.quizzes?.[0]) {
          if (mounted.current) {
            setQuiz(response.quizzes[0]);
            setLoading(false);
          }
          return;
        }
      } catch {
        // The fifth view and the quiz card can race by a few hundred ms.
      }
  ...
  if (completed) {
    return (
      <View style={[styles.card, fullscreen && styles.fullscreenCard, styles.doneCard]}>
        <Feather name="check-circle" size={28} color="#16A34A" />
        <Text style={[styles.doneText, fullscreen && styles.textOnDark]}>Brain break complete — keep exploring!</Text>
      </View>
    );
  }
  ...
  if (loading) {
    return (
      <View style={[styles.card, fullscreen && styles.fullscreenCard]}>
        ...
        <Text style={[styles.loadingText, fullscreen && styles.textOnDark]}>Getting your brain break ready…</Text>
      </View>
    );
  }
  ...
  if (loadError || !quiz) {
    return (
      <View style={[styles.card, fullscreen && styles.fullscreenCard]}>
        <Feather name="wifi-off" size={24} color={fullscreen ? '#FFFFFF' : colors.muted} />
        <Text style={[styles.question, fullscreen && styles.textOnDark]}>We need a quick quiz before you keep scrolling.</Text>
        <Pressable onPress={() => void loadRequiredQuiz()} style={styles.retry}>
        ...
        {options.map((option) => {
          ...
          return (
            <Pressable
              ...
              style={[styles.option, fullscreen && styles.optionDark, chosen && styles.optionChosen, correct && styles.optionCorrect, wrong && styles.optionWrong]}
            >
...
const styles = StyleSheet.create({
  card: { marginHorizontal: spacing.md, marginVertical: spacing.md, padding: spacing.lg, borderRadius: 18, backgroundColor: '#F8FAFC', borderWidth: 1, borderColor: '#DBEAFE', gap: 14 },
  fullscreenCard: { flex: 1, marginHorizontal: 18, marginVertical: 28, justifyContent: 'center', backgroundColor: '#0F172A', borderColor: '#334155' },
  doneCard: { alignItems: 'center', justifyContent: 'center' },
  doneText: { color: colors.ink, fontWeight: '800', fontSize: 16, textAlign: 'center' },
  badgeRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 10 },
  badge: { flexDirection: 'row', alignItems: 'center', gap: 5, alignSelf: 'flex-start', backgroundColor: colors.brand, borderRadius: radius.pill, paddingHorizontal: 9, paddingVertical: 5 },
  ...
  question: { color: colors.ink, fontWeight: '800', fontSize: 18, lineHeight: 25, textAlign: 'center' },
  options: { gap: 9 },
  option: { minHeight: 46, borderRadius: 12, borderWidth: 1, borderColor: '#D7DEE8', backgroundColor: '#FFFFFF', justifyContent: 'center', paddingHorizontal: 14, paddingVertical: 10 },
  ...
  retry: { alignSelf: 'center', backgroundColor: colors.brand, borderRadius: radius.pill, paddingHorizontal: 18, paddingVertical: 10 },
